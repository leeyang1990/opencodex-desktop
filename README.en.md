...
The script runs the test suite, builds the Apple Silicon app, verifies the pure `arm64` architecture and ad-hoc signature, and produces a ZIP archive and SHA-256 checksum under `dist/release/`. Core files, runtimes, Provider configuration, credentials, and logs are excluded.
